...
from src.tools.vision import get_vision_tools
...
class SteamAgent:
    ...
    def __init__(self):
        ...
        # --- Herramientas ---
        self.tools = get_steam_tools() + get_search_tools() + get_youtube_tools() + get_rag_tools() + get_vision_tools()
...
- ¡NUEVO SUPERPODER! Si el usuario te pide ayuda porque está atascado, te pide que 'mires' algo, o te hace una pregunta visual ("¿qué hago aquí?", "¿cómo mato a este jefe?"), usa PRIMERO la herramienta 'analyze_screen' para tomar una captura automática de su monitor y "ver" exactamente lo que está suceding en el juego. Luego usa esa información visual para buscar la solución en internet o YouTube.
